...
class GeoDBDownload extends Command
{
     ...
     * @var string
     */
    protected $signature = 'geodb:download';
     ...
     * @var string
     */
    protected $description = 'Download required geonames databases';
    ...
    public function handle()
    {
        ...
        $this->info("Updating GeoDB databases...");
...
//                }
            ...
            if ($recordCount) {
                DB::table('countries')->upsert($records, ['cc'], ['name', 'lang']);
            }
            DB::commit();
        });
        ...
        $countries = config('geodb.countries', 'all');
...
//            echo "-> ${country} [{$countryCount}/{$countryNumber}] ...";
            ...
            if (!file_exists($countryDataFile) || config('geodb.update', false)) {
...
//                    }
                ...
                if ($class == 'P' && ($code == 'PPLC' || $code == 'PPL' || $code == 'PPLA' || $code == 'PPLA2' || $code == 'PPLA3')) {
                    ...
                    $numCities++;
                    $totalCities++;
                } else if ($class == 'A' && ($code == 'RGN' || $code == 'ADM1' || $code == 'ADM1H')) {
                    $states[$stateCount] = [
                        'id' => $fields[0],
                        'name' => $fields[1],
                        'ascii_name' => $fields[2],
